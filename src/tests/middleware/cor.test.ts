...
test('Enable cor should inject headers', () => {
    ...
    expect(mockResponse.getHeader('Access-Control-Allow-Methods')).toBe('GET, POST, DELETE')
    expect(mockResponse.getHeader('Access-Control-Allow-Headers')).toBe('Content-Type,Authorization')
})